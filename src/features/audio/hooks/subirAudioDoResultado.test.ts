 ...
 * `if (!exigeAudio || linhaFinal.audio_path)`. Essa linha só é segura porque
 ...
 *   2. SEMPRE devolve uma `ResultadoRow`, e devolve a linha SEM `audio_path`
 ...
 * "por segurança" num caminho de falha, com `audio_path` preenchido de antes,
...
describe('subirAudioDoResultado — o contrato do qual o gate depende', () => {
  it('sem sessão: devolve a linha sem audio_path, e não lança', async () => {
    const set = setadores();
    const original = linha({ user_id: null });

    const devolvida = await subirAudioDoResultado(original, new Blob(['x']), set);

    expect(devolvida.audio_path).toBeNull();
    expect(set.chamadas).toContain('sem-conta');
  });

  it('sem blob: devolve a linha sem audio_path, e não lança', async () => {
    const set = setadores();
  ...
  it('o gate barraria a entrega nos dois casos', async () => {
    ...
    // audio_path, este caso quebra antes de alguém receber batalha muda.
    const semSessao = await subirAudioDoResultado(linha({ user_id: null }), new Blob(['x']), setadores());
    ...
    for (const devolvida of [semSessao, semBlob]) {
      expect(!exigeAudio || devolvida.audio_path).toBeFalsy();
    }
  });
});
