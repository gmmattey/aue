...
## D8 — Plataforma (premissa reafirmada, não decisão nova)

**PWA apenas. Não há e nunca houve plano de versão nativa para Android ou iOS.**

Registro de correção: esta sempre foi a premissa de Luiz. A hipótese de app nativo foi introduzida pela equipe de avaliação, não pelo produto — Thiago registrou `MediaRecorder` em PWA no iOS como limitado e levantou "pode forçar app nativo" como contingência técnica; Rafael converteu isso no risco R1; Rian trouxe a exigência de App Review 1.2 da Apple presumindo distribuição em loja. Nada disso partiu de Luiz.

Consequências — algumas delas corrigem o enquadramento com que D2 e D7 foram apresentadas:

- **A PWA sai do "fora de escopo" e vira o produto.** `manifest.json:13-15` aponta para um diretório `icons/` que não existe; não há service worker, então `offline.html` é inalcançável; o manifest é referenciado por uma única tela (`landing.html:7`). O que Rafael classificou como descartável passa a ser item central do MVP.
- **A cobrança deixa de ser IAP.** Sem App Store e Google Play, a assinatura passa por gateway próprio (Stripe, Pix ou equivalente). Isso **remove** o obstáculo citado em D7: o reembolso passa a ser controlado pelo Auê!, não pelas lojas. A reconciliação de estado de assinatura com Apple/Google descrita no desenho de backend deixa de existir.
- **D2 perde o driver de loja, mas permanece.** A exigência da Apple (App Review 1.2) deixa de se aplicar. Denúncia, bloqueio e contato publicado continuam necessários por D1 (13+ sob LGPD, critério de melhor interesse) e porque `legal.html:76` promete moderação.
- **A classificação etária das lojas some.** A lacuna L4 (palavrão) perde a dimensão de loja; resta apenas a dimensão jurídica sob D1.
- **Push no iOS** exige iOS 16.4+ e instalação na tela de início. D5 já tirou notificações do MVP, então não há impacto imediato — mas o teto é mais baixo se um dia entrarem.

**Risco criado, e é o mais sério até aqui:** a captura de áudio no iOS Safari deixa de ter rota de escape. Thiago já havia registrado `MediaRecorder` em PWA no iOS como notoriamente limitado, e a saída prevista era migrar para nativo. Essa saída foi fechada. O spike de áudio muda de pergunta: não é mais "PWA serve ou exige nativo?", e sim **"a captura funciona em PWA no iOS — e se não funcionar, qual é o plano?"**. Se a resposta for negativa, o produto fica sem iOS ou sem gravação no iOS, e isso é decisão de continuidade do projeto, não detalhe técnico.
...
Estava embutido em D1; a escolha de 13+ o torna passivo, mas não houve decisão de remover ou manter. Afeta `home.html:143`, `julgando.html:69`, `login.html:85`, `permissao.html:72`, `permissao-negada.html:77`. Com D8 (PWA apenas), a dimensão de classificação etária nas lojas deixa de existir; resta a dimensão jurídica sob D1.
...
- ~~Consertar a PWA~~ — **revogado por D8.** Com PWA apenas, manifest, ícones, service worker e o caminho offline passam a ser itens centrais do MVP, não descarte. Segue fora do escopo do *protótipo*, mas entra no escopo do produto.
